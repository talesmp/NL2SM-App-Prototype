...
from app_helpers import (
    ...
    handle_all_in_one,
    handle_distill_and_all_in_one, 
    handle_all_in_one_original_and_distilled
)
from app_helpers import state  # So we have a reference if needed
...
from db_utils import initialize_db, DB_PATH
...
def create_ui():
    """
    The original UI from your code. Call initialize_db() here
    to ensure your DB is created once the app starts.
    """
    initialize_db()

    with gr.Blocks() as app:
        with gr.Row():
            with gr.Column(scale=2): # Sidebar
                ...
                * Finally, accounts whose holders are bank branches pay neither annual fee nor transfer commission in any bank. 
                """
                srs_text = gr.TextArea(
                    label="SRS Textual Description",
                    lines=24,
                    max_lines=24,
                    value=srs_placeholder
                )

                all_in_one_btn = gr.Button("All in One (Original SRS)")
                distill_and_all_in_one_btn = gr.Button("All in One (Distilled SRS)")
            ...
            with gr.Column(scale=4): # Main area
                ...
                with gr.Row(): # PlantUML script and UML image
                    plantuml_script = gr.TextArea(
                        label="PlantUML Script",
                        lines=12,
                        max_lines=12,
                ...
                with gr.Row(): # Similarity analysis
                    similarity_result = gr.TextArea(
                        label="Similarity Analysis",
                        lines=12
                    )

                with gr.Row(): # Distilled SRS
                    distilled_srs_box = gr.TextArea(
                    label="Distilled SRS", 
                    lines=10, 
                    max_lines=10
                )
        ...
        send_btn.click(
            ...
            outputs=[srs_text, chatbot, plantuml_script]
        )
        ...
        all_in_one_btn.click(
            fn=lambda srs, prov, mdl, temp, chat: handle_all_in_one(
                srs_type="original",
                srs_text=srs,
                provider=prov,
                model=mdl,
            ...
            inputs=[srs_text, provider, model, temperature, chatbot],
            outputs=[srs_text, chatbot, plantuml_script, uml_image, similarity_result]
        )

        distill_and_all_in_one_btn.click(
            fn=handle_distill_and_all_in_one,
            inputs=[
                srs_text,    # Original SRS 
                provider,
                model,
                temperature,
                chatbot
            ...
            outputs=[
                distilled_srs_box,  # 1) The newly distilled SRS
                srs_text,           # 2) new_srs_text from handle_all_in_one
                chatbot,            # 3) updated chatbot
                plantuml_script,    # 4) UML script
                uml_image,          # 5) UML diagram path
                similarity_result   # 6) similarity text
            ]
        )
...
def create_new_ui():
    ...
    with gr.Blocks(fill_width=True) as app:
        ...
        with gr.Row(): # Original SRS text and Distilled SRS text
            srs_placeholder = """
                Suppose the financial system of a country is composed of banks. 
                * Each bank has branches, which may be spread throughout the country, but there may not be more than three branches of the same bank in the same city. 
                * The structure of a bank's branches is hierarchical, so that a branch may have subordinate branches (of the same bank), either in the same city or in other cities. 
            ...
            with gr.Column(): 
                srs_text_original = gr.TextArea(
                    label="Original SRS Textual Description",
                    lines=18,
                    max_lines=18,
                    value=srs_placeholder
                )
                # all_in_one_original_and_distilled_btn = gr.Button("Send to LLM (All in One)")
                all_in_one_original_and_distilled_btn = gr.Button("Send to LLM (Orig + Dist)")
            with gr.Column():
        ...
        with gr.Row():  # Original and Distilled PlantUML scripts
            with gr.Column():
                plantuml_script_original = gr.TextArea(
                    label="PlantUML Script (SRSo)",
                    lines=12,
                    max_lines=12,
                    show_copy_button=True
                )
            with gr.Column():
                plantuml_script_distilled = gr.TextArea(
                    label="PlantUML Script (SRSd)",
                    lines=12,
                    max_lines=12,
                    show_copy_button=True
                )

        with gr.Row():  # Original and Distilled UML images
            with gr.Column():
                uml_image_original = gr.Image(
                    type="pil",
                    label="PlantUML Diagram (SRSo)",
                    height=306,
                    show_download_button=True,
                    show_fullscreen_button=True
                )
        ...
        with gr.Row():  # Original and Distilled Similarity Analysis
            with gr.Column():
                similarity_result_original = gr.TextArea(
                    label="Similarity Analysis (SRSo)",
                    lines=12
                )
            with gr.Column():
                similarity_result_distilled = gr.TextArea(
                    label="Similarity Analysis (SRSd)",
                    lines=12
                )
